...
public class rest2Controller {
	...
	public String rest6() {
		...
		/*
		 * 단순텍스트 abc, qwe, name1, CUS return "abc, qwe, name1, CUS";
		 * 
		 * 많은 데이터 전달 -> 표현 규칙 -> JSON { "id" : "abc", "pw" : "qwe", "name" : "name1",
		 * "userType" : "CUS" }
		 */

		/*
		 * JSON 포맷으로 return을 하려면? 1) 직접 포맷대로 작성
		 * 
		 * 
	...
	@GetMapping("/rest/rest7") // 사전 조건 : jackson 라이브러리가 존재해야 함
	public User rest7() {
		// user 객체 리턴!
		User user = new User();
		user.setId("abc2");
		user.setPw("qwe2");
		user.setName("name157531");
		user.setUserType(CommonCode.USER_USERTYPE_CUSTOMER);
	...
	public List<ApiMenu> rest9() {
		...
		menuList.add(new ApiMenu("아메", 2500));
		menuList.add(new ApiMenu("카페라떼", 2500));
	...
	public ApiDelivery rest10() {
		...
		apiDelivery.setDestination("로하스7층");
		apiDelivery.setPhone("010-1234-5678");

		ApiStore apiStore = new ApiStore();
		apiStore.setName("롯데리아");
		apiStore.setAddress("대흥동");
		apiStore.setTel("02-111-1111");

		apiDelivery.setApiStore(apiStore);

		List<ApiMenu> menuList = new ArrayList<ApiMenu>();
		menuList.add(new ApiMenu("버거", 3000));
		menuList.add(new ApiMenu("버거", 3000));
		menuList.add(new ApiMenu("버거", 3000));
