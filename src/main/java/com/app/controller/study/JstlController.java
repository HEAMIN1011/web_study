...
public class JstlController {
	...
	public String listTest(@RequestParam String type, Model model) {
		 ...
		 * 
		 * List<String> msgList = new ArrayList<String>(); for(int i=1; i<=15; i++)
		 ...
		 * model.addAttribute("type", type);
		 * 
		 * if(type.equals("str")) { model.addAttribute("msg", "스트링 리스트입니다.");
		 * 
		 * List<String> msgList = new ArrayList<String>(); for(int i=1; i<=15; i++)
		 * msgList.add("스트링 리스트입니다.");
		 * 
		 * model.addAttribute("msgList", msgList); }
		 * 
		 * if(type.equals("member")) { List<Member> memberList = new
		 * ArrayList<Member>();
		 * 
		 * for(int i=1; i<=5; i++) { memberList.add(new Member("아이디"+i, "비번"+i,
		 * "이름"+i)); }
	...
	/*
	 * @GetMapping("/jstl/listTest/str") public String listTestStr(Model model) {
	 * model.addAttribute("msg", "스트링 리스트입니다.");
	 * 
	 * List<String> msgList = new ArrayList<String>(); for(int i=1; i<=15; i++)
	 * msgList.add("스트링 리스트입니다.");
	 ...
	 * return "jstl/str"; }
	 * 
	 * @GetMapping("/jstl/listTest/member") public String listTestMember(Model
	 * model) {
	 * 
	 * List<Member> memberList = new ArrayList<Member>();
	 * 
